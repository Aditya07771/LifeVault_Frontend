  ...
  const fetchStories = async () => {
    ...
    try {
      const [myRes, receivedRes] = await Promise.all([
        storyAPI.getMyStories().catch(() => ({ data: { data: [] } })),
        storyAPI.getReceived().catch(() => ({ data: { data: [] } })),
      ]);
      setMyStories(myRes.data?.data || []);
      setReceivedStories(receivedRes.data?.data || []);
    } catch (e) {
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen space-y-6 pb-12">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl">
          ...
          <div className="relative z-10 px-6 py-8 sm:px-10 sm:py-12">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-white/90 text-sm font-semibold tracking-wide">Dashboard Overview</span>
            </div>
            ...
            <p className="text-white/95 text-base sm:text-lg max-w-2xl mb-8 leading-relaxed">
              Your memories are safe and encrypted. Continue preserving your most precious moments.
            </p>
        ...
        {/* Stats Section */}
        <section>
          <StatsCards stats={stats} />  {/* Pass stats directly */}
        </section>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="relative">
              <p className="text-sm text-gray-500 font-semibold mb-1">Total Memories</p>
              <p className="text-3xl font-black text-gray-900 mb-2">{safeStats.totalMemories}</p>
          ...
          >
            ...
            <div className="relative">
              <p className="text-sm text-gray-500 font-semibold mb-1">Active Quests</p>
              <p className="text-3xl font-black text-gray-900 mb-2">{safeStats.activeQuests}</p>
          ...
          >
            ...
            <div className="relative">
              <p className="text-sm text-gray-500 font-semibold mb-1">Your Stories</p>
              <p className="text-3xl font-black text-gray-900 mb-2">{myStories.length}</p>
          ...
          >
            <div className="relative">
              <p className="text-sm text-gray-500 font-semibold mb-1">Last Sync</p>
              <p className="text-sm font-bold text-gray-700 mb-2">Just now</p>
              <RefreshCw className={`w-5 h-5 text-indigo-600 ${loading ? 'animate-spin' : 'group-hover:rotate-180'} transition-all duration-500`} />
            </div>
          </button>
        </section>
        ...
        <section className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 font-medium">Time-locked memories for special moments</p>
            </div>
          </div>
            ...
            <div className="bg-white border-2 border-gray-100 rounded-3xl overflow-hidden hover:border-indigo-100 hover:shadow-2xl transition-all duration-300">
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 px-6 py-5">
                <div className="flex items-center justify-between">
                  ...
                  >
                    <RefreshCw className={`w-4 h-4 text-white ${loadingStories ? 'animate-spin' : ''}`} />
                  </button>
                </div>
              </div>
              ...
              <div className="p-6">
                {loadingStories ? (
                  ...
                  <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-2xl bg-gradient-to-br from-gray-50 to-white">
                    ...
                    <p className="text-sm font-bold text-gray-600 mb-1">No stories yet</p>
                    <p className="text-xs text-gray-400 mb-4">Create your first story to get started</p>
                    <button
                      onClick={() => setShowStoryModal(true)}
                    ...
                    {myStories.slice(0, 3).map((story) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={`flex-shrink-0 text-[10px] uppercase font-black px-3 py-1 rounded-full ${story.status === 'published'
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 text-xs text-gray-500 font-semibold">
                            ...
                            {story.totalChapters || story.chapters?.length || 0} chapters
                          </div>
              ...
              <div className="p-6">
                {loadingStories ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-sm text-gray-400 font-medium">Checking stories...</p>
                  </div>
                ) : receivedStories.length === 0 ? (
                  <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-2xl bg-gradient-to-br from-gray-50 to-white">
                    <Lock className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                    <p className="text-sm font-bold text-gray-600 mb-1">No received stories</p>
                    <p className="text-xs text-gray-400">Stories shared with you will appear here</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className="flex-shrink-0 text-xs font-black text-amber-700 bg-amber-100 border border-amber-200 px-3 py-1 rounded-full">
                            {Math.round(story.progress?.progress || 0)}%
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-gray-500 font-semibold flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" />
                            {story.progress?.nextLocked?.reason || 'Tap to unlock'}
                          </p>
        ...
        <section className="space-y-6">
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-gray-500 mt-2 font-medium">Browse through your cherished moments</p>
            </div>
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm font-black text-gray-700 uppercase tracking-wider">Filter by type</span>
            </div>
                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-shrink-0 px-6 py-3 rounded-xl font-bold text-sm transition-all duration-200 border-2 ${isActive
...
// Helper component for bookmark icon
const BookmarkIcon = () => (
